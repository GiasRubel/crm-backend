...
export class OpportunitiesController {
  ...
  findAll(
    ...
    return this.opportunitiesService.findAll(query, user.sub);
  }

  /** Kanban board: one column per pipeline stage, with value totals. */
  @Get('board')
  @Roles(AppRole.Admin, AppRole.Administrator, AppRole.User)
  getBoard(@CurrentUser() user: KeycloakJwtPayload) {
    return this.opportunitiesService.getBoard(user.sub);
  }

  @Get('stats')
  @Roles(AppRole.Admin, AppRole.Administrator, AppRole.User)
  getStats(@CurrentUser() user: KeycloakJwtPayload) {
    return this.opportunitiesService.getStats(user.sub);
  }

  @Get(':id')
  @Roles(AppRole.Admin, AppRole.Administrator, AppRole.User)
  findOne(@Param('id') id: string, @CurrentUser() user: KeycloakJwtPayload) {
    return this.opportunitiesService.findOne(id, user.sub);
  }
  ...
  assign(@Param('id') id: string, @Body() dto: AssignOpportunityDto) {
    return this.opportunitiesService.assign(id, dto);
  }
