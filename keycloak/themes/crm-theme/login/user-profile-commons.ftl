<#--
  Declarative user-profile fields (register, update-profile, idp-review-profile).

  Same logic as the base theme — Keycloak drives which attributes exist — but the
  wrapper markup is ours: one `.crm-field` per attribute, the required marker
  inside the <label>, and no nested label/input wrapper divs. The input tags
  themselves are unchanged so every Keycloak input type keeps working.
-->
<#macro userProfileFormFields>
	...
	<#list profile.attributes as attribute>
		...
		<#if group != currentGroup>
			...
			<#if currentGroup != "">
				...
				>
					...
					<h2 id="header-${attribute.group.name}" class="crm-fieldset__title">${groupHeaderText}</h2>
					...
					<#if groupDisplayDescription != "">
						...
						<p id="description-${group.name}" class="crm-fieldset__desc">${groupDescriptionText}</p>
					</#if>
				</div>
			</#if>
		</#if>
		...
		<div class="crm-field<#if messagesPerField.existsError('${attribute.name}')> crm-field--error</#if>">
			<label for="${attribute.name}" class="crm-label">
				${advancedMsg(attribute.displayName!'')}<#if attribute.required><span class="crm-required" aria-hidden="true">*</span></#if>
			</label>
			<#if attribute.annotations.inputHelperTextBefore??>
				<div class="crm-field__help" id="form-help-text-before-${attribute.name}" aria-live="polite">${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextBefore))?no_esc}</div>
			</#if>
			<@inputFieldByType attribute=attribute/>
			<#if messagesPerField.existsError('${attribute.name}')>
				<span id="input-error-${attribute.name}" class="crm-field__error" aria-live="polite">
					${kcSanitize(messagesPerField.get('${attribute.name}'))?no_esc}
				</span>
			</#if>
			<#if attribute.annotations.inputHelperTextAfter??>
				<div class="crm-field__help" id="form-help-text-after-${attribute.name}" aria-live="polite">${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextAfter))?no_esc}</div>
			</#if>
		</div>
...
<#macro inputTag attribute value>
	<input type="<@inputTagType attribute=attribute/>" id="${attribute.name}" name="${attribute.name}" value="${(value!'')}" class="${properties.kcInputClass!}"
		...
		<#if attribute.annotations.inputTypePlaceholder??>placeholder="${attribute.annotations.inputTypePlaceholder}"<#else>placeholder="${advancedMsg(attribute.displayName!'')}"</#if>
...
<#macro textareaTag attribute>
	<textarea id="${attribute.name}" name="${attribute.name}" class="${properties.kcTextareaClass!}"
		aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
		<#if attribute.readOnly>disabled</#if>
...
<#macro selectTag attribute>
	<select id="${attribute.name}" name="${attribute.name}" class="${properties.kcInputClass!} crm-input--select"
		aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
		<#if attribute.readOnly>disabled</#if>
